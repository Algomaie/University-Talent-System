                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                        ...
                        <form action="{{ route('student.notifications.read-all') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="text-xs text-red-600 hover:text-red-800" onclick="return confirm('{{ __('Are you sure you want to delete all notifications? This action cannot be undone.') }}')">
                                {{ __('Delete All') }}
                            </button>
                        </form>
                        @endif
